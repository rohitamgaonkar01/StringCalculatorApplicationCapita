...
import com.StringCalculator.functors.Map6;
import com.StringCalculator.functors.Map7;
import com.StringCalculator.functors.Map8;
import com.StringCalculator.functors.Pair;
import com.StringCalculator.functors.Tuple3;
import com.StringCalculator.functors.Tuple4;
import com.StringCalculator.functors.Tuple5;
...
public final class Parsers {
  ...
  /** A { Parser} that succeeds only if EOF is met. Fails with {@code message} otherwise. */
  ...
  /** A { Parser} that always fails with {@code message}. */
  ...
  /**
   * A { Parser} that always succeeds and invokes {@code runnable}.
   */
  @Deprecated
  ...
  /** Converts a parser of a collection of { Token} to a parser of an array of {@code Token}.*/
  ...
  /**
   * A { Parser} that takes as input the array of { Token} returned from {@code lexer},
  ...
  /** A { Parser} that always returns {@code v} regardless of input. */
  ...
  /**
   * A { Parser} that runs 2 parser objects sequentially. {@code p1} is executed,
  ...
  /**
   * A { Parser} that sequentially runs {@code p1} and {@code p2} and collects the results in a
   * { Pair} object. Is equivalent to { #tuple(Parser, Parser)}.
   *
  ...
  /**
   * A { Parser} that sequentially runs {@code p1} and {@code p2} and collects the results in a
   * { Pair} object. Is equivalent to { #pair(Parser, Parser)}.
   *
   * @deprecated Prefer to converting to your own object with a lambda.
   */
  @Deprecated
  ...
  /**
   * A { Parser} that sequentially runs {@code parsers} one by one and collects the return
   * values in an array.
   */
  ...
  /**
   * Equivalent to { Parser#between(Parser, Parser)}. Use this to list the parsers in the
   * natural order.
   */
  ...
  /**
   * A { Parser} that runs {@code p1} and {@code p2} sequentially
  ...
  /** A { Parser} that runs {@code parsers} sequentially and discards the return values. */
  ...
  /**
   * A { Parser} that tries each alternative parser in {@code alternatives}.
   * 
   * <p> Different than { #alt(Parser[])}, it requires all alternative parsers to have
   * type {@code T}.
   */
  ...
  /**
   * A { Parser} that runs both {@code p1} and {@code p2} and selects the longer match.
  ...
  /**
   * A { Parser} that runs every element of {@code parsers} and selects the longest match.
  ...
  /**
   * A { Parser} that runs both {@code p1} and {@code p2} and selects the shorter match.
   * If both matches the same length, the first one is favored.
   */
  @SuppressWarnings("unchecked")
  ...
  /**
   * A { Parser} that runs every element of {@code parsers} and selects the shortest match.
   * If two matches have the same length, the first one is favored.
   */
  ...
  /** A { Parser} that fails and reports that {@code name} is logically expected. */
  ...
  /** A { Parser} that fails and reports that {@code name} is logically unexpected. */
   ...
   * { TokenMap#map(Token)} method returns null, an unexpected token error occurs;
   ...
   * a new { java.util.Iterator} object each time the parser runs.
   */
